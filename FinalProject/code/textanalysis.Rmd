...
Let's look at the words with the highest mean TF-IDF values:
...
First let's do a boosting model with 5 trees. This is not a lot of trees, so we'd expect that the model wouldn't do so well with prediction on our test set.
...
estimatepoints <- function(country, price, description, province, variety) {
  ...
  dtm_newwine <- CreateDtm(newwine$description, #create DTM for new wine
